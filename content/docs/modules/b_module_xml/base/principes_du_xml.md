...
Un même document XML peut être enregistré sous plusieurs jeux de caractères. Cependant, il faut que la déclaration corresponde au format utilisé lors de l'enregistrement du fichier, sinon les caractères accentués (par exemple) seront pervertis.
...
Les fichiers de texte sont souvent organisés en lignes. Selon le système d'exploitation, il y a plusieurs façons de représenter une fin de ligne. Les systèmes d'exploitation Microsoft utilisent deux caractères (« retour de charriot » et « nouvelle ligne ») à la fin de chaque ligne. Les autres systèmes utilisent généralement un seul caractère (« nouvelle ligne »). Les processeurs XML sont requis de normaliser la situation et de remplacer les deux caractères utilisés par les systèmes Microsoft par un seul caractère lors de la lecture du fichier. Ainsi, peu importe comment l'on note les fins de ligne, un processeur XML ne verra qu'un caractère « nouvelle ligne ».
